...
if (!isset($_SESSION['dealer_general_login'])) {
    ...
    $dataUser = $conn->prepare("SELECT * FROM dealer_general WHERE id = $user_id");
    $dataUser->execute();
...
if (isset($_POST['submit_order'])) {
    $note = $_POST['note'];
    $cus_id = $_SESSION['dealer_general_login'];
    // $shipping_name = $_POST["shipping_name"];
    $shipping_name = "-";
    ...
    $last_price = ($final_price + $tax) + $ship_cost;
        ...
        if (in_array($fileActExt1, $allow)) {
            if ($slip_img['size'] > 0 && $slip_img['error'] == 0) {
                if (move_uploaded_file($slip_img['tmp_name'], $filePath1)) {
                    ...
                    $order = $conn->prepare("INSERT INTO orders(cus_id, cus_name, address, tel, email, urole, slip_img, total_price,shipping_company, parcel_code, status_order ,payment_method,note)
                                            VALUES(:cus_id, :cus_name, :address, :tel, :email, :urole,  :slip_img,  :total_price, :shipping_company, :parcel_code, :status_order , :payment_method, :note)");
                    ...
                    $order->bindParam(":urole", $rowUser["urole"]);
                    ...
                    if ($order) {
                        //vV0QcPW0su13KTbdcaHBp6Uf7E1GzFbk0dJVPGQ0g4T
                        $sToken = "vV0QcPW0su13KTbdcaHBp6Uf7E1GzFbk0dJVPGQ0g4T";
                        $sMessage = "แจ้งเตือนข้อมูลการสั่งซื้อ!!!\r\n";
                        $sMessage .= "คุณ " . $p_name . " " . "ได้ทำการสั่งซื้อสินค้า\r\n";
                        $sMessage .= "หมายเลขคำสั่งซื้อเลขที่: " . $id_order .  " \r\n";
                        $sMessage .= "ราคาทั้งหมด: " . number_format($last_price, 2)  . " บาท\r\n";
...
<body style="font-family: 'Kanit', sans-serif;">
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            <div class="order-address-box">
                ...
                <div class="order-box-input-address">
                    ...
                    <a href="../dealer_general/dealer_gen_profile_address"><button type="button" class="order-btn-address-submit">เปลี่ยน</button></a>
                    ...
                    <table class="table table-hover table-order-style">
                        ...
                        for ($i = 0; $i < count($product); $i++) {  ?>
                            ...
                            <tbody class="align-middle">
                                <tr>
                                    <th>
                                        <img class="img-list-order" src="../admin/p_img/<?= $final_p[$i][0][10] ?>" alt="">
                                    </th>
    ...
    <script type="text/javascript">
        ...
        $('#select-logis').change(function() {
            ...
            $.ajax({
                type: "POST",
                url: '../dealer_general/dealer_gen_order.php',
                data: {

...
if (isset($_POST['submit'])) {
    ...
    if (empty($addressed)) {
        ...
        header("refresh:0.0000000000001; url=profile_address");
    } else if (empty($provinces)) {
        echo "<script>alert('กรุณากรอกข้อมูลให้ครบถ้วน')</script>";
        header("refresh:0.0000000000001; url=profile_address");
    } else if (empty($amphures)) {
        echo "<script>alert('กรุณากรอกข้อมูลให้ครบถ้วน')</script>";
        header("refresh:0.0000000001; url=profile_address");
        ...
        try {
            if (!isset($_SESSION['error'])) {
                $sql = $conn->prepare("UPDATE users SET address1 = :address1 WHERE id = :id");
                $sql->bindParam(":id", $id);
                ...
                if ($sql) {
                    ...
                    echo "<meta http-equiv='Refresh' content='0.001; url=profile_address'>";
                }
            }
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="profile_address_form" method="POST">
                    <div class="box-1">
